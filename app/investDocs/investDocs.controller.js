...
export const getDocuments = asyncHandler(async (req, res) => {
	...
	const totalDocuments = await prisma.investDocument.count()

	const docs = await prisma.investDocument.findMany({
		skip: rangeStart,
...
export const getDocument = asyncHandler(async (req, res) => {
	const document = await prisma.investDocument.findUnique({
...
export const createDocument = asyncHandler(async (req, res) => {
	...
	const document = await prisma.investDocument.create({
		data: {
...
export const updateDocument = asyncHandler(async (req, res) => {
	...
	try {
		const document = await prisma.investDocument.update({
			where: {
				id: +req.params.id
			},
...
export const deleteDocument = asyncHandler(async (req, res) => {
	try {
		const document = await prisma.investDocument.delete({
			where: {
				id: +req.params.id
			}
		})
